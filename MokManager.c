...
static EFI_STATUS mok_tml_prompt(void *MokTML, UINTN MokTMLSize)
{
	...
	if (ret == 0) {
		LibDeleteVariable(L"MokTML", &SHIM_LOCK_GUID);
		return EFI_ABORTED;
	}

	if (var->MokTMLState == 0) {
		efi_status = gRT->SetVariable(L"MokTMLState", &SHIM_LOCK_GUID,
					      EFI_VARIABLE_NON_VOLATILE |
					      EFI_VARIABLE_BOOTSERVICE_ACCESS,
					      1, &dbval);
		if (EFI_ERROR(efi_status)) {
			console_notify(L"Failed to set TML state");
			return efi_status;
		}
	} else {
		efi_status = gRT->SetVariable(L"MokTMLState", &SHIM_LOCK_GUID,
					      ...
					      0, NULL);
		if (EFI_ERROR(efi_status)) {
			console_notify(L"Failed to delete TML state");
			return efi_status;
		}
	}
				 ...
				 void *MokTML, UINTN MokTMLSize)
{
	...
	while (choice >= 0) {
		...
		case MOK_CHANGE_TML:
			if (!MokTML) {
				console_print(L"MokManager: internal error: %s",
					L"MokTML was ! NULL bs is now NULL\n");
				ret = EFI_ABORTED;
				goto out;
			}
...
static EFI_STATUS check_mok_request(EFI_HANDLE image_handle)
{
	...
	efi_status = get_variable(L"MokTML", (UINT8 **) & MokTML, &MokTMLSize,
				  SHIM_LOCK_GUID);
	if (!EFI_ERROR(efi_status)) {
		efi_status = LibDeleteVariable(L"MokTML", &SHIM_LOCK_GUID);
		if (EFI_ERROR(efi_status))
			console_notify(L"Failed to delete MokTML");
	} else if (EFI_ERROR(efi_status) && efi_status != EFI_NOT_FOUND) {
		console_error(L"Could not retrieve MokTML", efi_status);
	}
